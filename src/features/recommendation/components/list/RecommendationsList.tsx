// src/features/recommendations/components/list/RecommendationsList.tsx

/**
 * RecommendationsList Component
 * Main container for predictions list with grid/table views
 */
...
interface RecommendationsListProps {
    patientId?: string; // Optional - if provided, filter by this patient
}
...
export function RecommendationsList({ patientId }: RecommendationsListProps = {}) {
    ...
    const { data, isLoading, error } = useRecommendations({
        page,
        per_page: perPage,
        patient_id: patientId, // Filter by patient if provided
    });
    ...
    const sortedPredictions = data?.predictions ? [...data.predictions].sort((a, b) => {
        return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
    }) : [];

    // Calculate stats
    const totalPredictions = data?.pagination?.total || 0;
    const avgConfidence = sortedPredictions.length > 0
        ? sortedPredictions.reduce((sum, p) => sum + parseFloat(p.confidence_score), 0) / sortedPredictions.length
        : 0;

    // Determine page title based on context
    const pageTitle = patientId ? 'Patient Predictions' : 'AI Predictions';
    const pageSubtitle = patientId
            ...
            <div className="summary-cards">
                <div className="summary-card">
                    ...
                    <div className="summary-card-content">
                        <p className="summary-card-label">Total Predictions</p>
                        <p className="summary-card-value">{totalPredictions}</p>
                    </div>
                </div>

                {totalPredictions > 0 && (
                    <div className="summary-card">
                        <div className="summary-card-icon confidence">
                            <i className="fa-solid fa-chart-line"></i>
                        </div>
                        <div className="summary-card-content">
                            <p className="summary-card-label">Avg Confidence</p>
                            <p className="summary-card-value">{avgConfidence.toFixed(1)}%</p>
                        </div>
                    </div>
                )}
            </div>
                            ...
                            {viewMode === 'table' && (
                                <div className="predictions-table">
                                    <div className="table-header">
                                        ...
                                        <div className="table-cell">Reduction</div>
