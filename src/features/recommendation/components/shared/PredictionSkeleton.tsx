...
function SkeletonBlock({ className }: { className?: string }) {
    return <div className={`bg-white/10 rounded-none animate-pulse ${className || ''}`} />;
}
...
export function PredictionSkeleton({ variant = 'card', count = 1 }: PredictionSkeletonProps) {
    ...
    if (variant === 'card') {
        return (
            <>
                {items.map((i) => (
                    <div key={i} className="border border-white/10 bg-white/[0.04] rounded-none p-4 animate-pulse">
                        {/* Header */}
                        ...
                        {/* Treatment */}
                        <div className="bg-white/[0.02] border border-white/[0.06] rounded-none p-3 mb-3 flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-2.5 mb-3">
                            {[0, 1, 2, 3].map((j) => (
                                <div key={j} className="flex flex-col gap-1.5 p-2.5 bg-white/[0.02] rounded-none">
